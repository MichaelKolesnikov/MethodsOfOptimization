def knapsack(n: int, w: int, weight: list[int], cost: list[int]):
    ...
    print(fr"\[W_{n}({w_}) \Rightarrow x_{n}^o={x_o[n][w_]}\]")
    for n_ in range(n - 1, 0, -1):
        print(r"\[" + f"W_{n_}({w_} - {x_o[n_ + 1][w_]} * {weight[n_ + 1]})=", end='')
        w_ -= x_o[n_ + 1][w_] * weight[n_ + 1]
        print(f"{dp[n_][w_]}" + r"\text{при}" + f"x_{n_}^o={x_o[n_][w_]}" + r"\]")
...
def print_latex_matrix(matrix):
    latex_code = "\\begin{tabular}{" + "|c" * len(matrix[0]) + "|}\n"
    latex_code += "\\hline\n"
    for row in matrix:
...
def main():
    ...
    print("Веса:")
    print_latex_matrix([weight[1:]])
    print("Стоимости:")
    print_latex_matrix([cost[1:]])
    print("Решение:")
